...
import { registerOracleUi, renderOracleCall, renderOracleResult, withOracleUi } from "./widget";
...
export function registerOracleTool(pi: ExtensionAPI, deps: OracleToolDeps) {
	registerOracleUi(pi, deps.hasActiveOracle, deps.cancelActiveOracle);

	pi.registerTool({
		name: "oracle",
		label: "Oracle",
		description:
			"Ask a higher-reasoning agent for complex debugging, subtle logic review, architecture/refactor decisions, or deep analysis. Avoid oracle for routine edits, formatting, simple lookups, or day-to-day coding tasks. The oracle runs Codex in read-only mode and should not mutate files. The oracle result is rendered directly for the user and returned to the calling agent; do not repeat it verbatim unless the user explicitly asks.",
		promptSnippet:
			"Ask a higher-reasoning agent for opinions. The result is printed for the user and passed back to you; use it internally without restating it verbatim.",
		promptGuidelines: [
			"Use oracle when a complex bug, subtle logic review, architecture choice, or backwards-compatible refactor would benefit from an independent higher-reasoning second opinion.",
			"Avoid oracle for routine edits, formatting, simple questions, or normal code changes.",
			...
			"If it says IMPLEMENT, continue with the provided task and files.",
		],
		parameters: ORACLE_PARAMS,
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			return await withOracleUi(ctx, params.question, deps.formatDuration, async (ui) => {
				try {
					const result = await deps.runCodexOracle(params, ctx.cwd, signal, ui.update);
